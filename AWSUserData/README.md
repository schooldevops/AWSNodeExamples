...
UserData 는 AWS EC2 인스턴스가 런칭될때, 필요한 소프트웨어를 설치하는 Provisioning 명령들을 기술할 수 있다. 

EC2 인스턴스가 정상적으로 실행이 되고 나면, UserData가 살행되며, 기술한 UserData 스크립트 명령어를 통해서, 필요한 소프트웨어가 프로비저닝 된다. 

이번 아티클에서는 NodeJS 를 이용한 간단한 웹 어플리케이션을 작성하고, 이를 UserData를 통해서 실행될 수 있도록 작업해 보고자한다. 
